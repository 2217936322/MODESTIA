...
namespace Utils {
    ...
    static T* FindHudElement(const char* name) {
        static auto Fn = *reinterpret_cast<DWORD**>(PatternScan(GetModuleHandleA("client.dll"), ("B9 ? ? ? ? E8 ? ? ? ? 8B 5D 08")) + 1);

        static auto FindHudElement = reinterpret_cast<DWORD(__thiscall*)(void*, const char*)>(PatternScan(GetModuleHandleA("client.dll"), ("55 8B EC 53 8B 5D 08 56 57 8B F9 33 F6 39 77 28")));
        return (T*)FindHudElement(Fn, name);
    }

    void FullUpdate() {
        static auto fn = reinterpret_cast<std::int32_t(__thiscall*)(void*, std::int32_t)>(PatternScan(GetModuleHandleA("client.dll"), ("55 8B EC 51 53 56 8B 75 08 8B D9 57 6B FE 2C")));

        auto element = FindHudElement<std::uintptr_t*>(("CCSGO_HudWeaponSelection"));

        auto hudWeapons = reinterpret_cast<HudWeapons_t*>(std::uintptr_t(element) - 0xA0);
        if (hudWeapons == nullptr)
            return;

        if (!*hudWeapons->GetWeaponsCount())
            return;

        for (std::int32_t i = 0; i < *hudWeapons->GetWeaponsCount(); i++)
            i = fn(hudWeapons, i);
